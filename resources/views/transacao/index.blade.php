...
<table class="table table-bordered table-sm text-center">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <div class="text-end">
                    <button type="button" class="btn btn-outline-danger mx-1 delete" onclick="Delete('{{route("transacao.delete", ["id" => $transacao->id])}}');"><i class="fa-light fa-trash fa-sm"></i> Remover
                    </button>
                </div>
            </td>
        </tr>
                ...
                <form name="formFilterTran" method="get" action="{{url("transacao/")}}">
                    <div class="mb-3">
                        ...
                        <select id="produto" name="produto" class="form-select">
                            ...
                            @foreach($produtos as $produto)
                            <option value="{{$produto->id}}" {{$produto->id === $transacao->id_itm ? "selected" : ""}}>{{$produto->desc}}</option>
                            @endforeach
                        </select>
                        ...
                        <select id="armazem" name="armazem" class="form-select">
                            <option value="0">TODOS</option>
                            @foreach($armazens as $armazem)
                            <option value="{{$armazem->id}}" {{$armazem->id === $transacao->id_arm ? "selected" : ""}}>{{$armazem->desc}}</option>
                            @endforeach
                        </select>
                    </div>
