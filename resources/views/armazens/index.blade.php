...
@section('content')
    ...
    <div class="col-md-2 mb-2">
        <a class="btn btn-outline-success" href="{{url('armazens/create')}}" role="button"><i
        ...
        @foreach($armazens as $armazem)
            <form name="formEdtArm" method="post" action="{{url("armazens/$armazem->id")}}">
                ...
                <tr>
                    <td>{{$armazem->id}}</td>
                    <td>
                        <input type="text" id="desc" name="desc" class="form-control" value="{{$armazem->desc}}"
                    ...
                    <td>
                        <select id="emp" name="emp" class="form-select"
                            ...
                            @foreach($empresas as $empresa)
                                <option
                                    value="{{$empresa->id}}" {{$empresa->id === $armazem->id_emp ? "selected" : ""}}>{{$empresa->desc}}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
                        <input type="text" id="" name="cidade" class="form-control" value="{{$armazem->cidade}}"
                               onclick="show_btn({{$armazem->id}})">
                    </td>
                    <td>
